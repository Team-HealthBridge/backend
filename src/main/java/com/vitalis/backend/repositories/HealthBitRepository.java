...
public interface HealthBitRepository extends JpaRepository<HealthBit, Long> {
    ...
    @Query(value = "SELECT * FROM health_bits order by random()", nativeQuery = true)
    Page<HealthBit> findRandom(Pageable pageRequest);

    @Query(value = "SELECT * FROM health_bits WHERE category = :category order by random()", nativeQuery = true)
